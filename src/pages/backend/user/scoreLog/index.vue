<template>
	<div class="default-main ba-table-box">
		...
		<TableHeader
			...
			:quick-search-placeholder="t('quick Search Placeholder', { fields: t('user.moneyLog.User name') + '/' + t('user.moneyLog.User nickname') })"
			@action="baTable.onTableHeaderAction"
		>
			<el-button v-if="!_.isEmpty(state.userInfo)" v-blur class="table-header-operate">
				<span class="table-header-operate-text">{{ state.userInfo.username + '(ID:' + state.userInfo.id + ') ' + t('user.scoreLog.integral') + ':' + state.userInfo.score }}</span>
			</el-button>
		</TableHeader>
