	...
	<div class="login">
							...
							<el-form ref="formRef" @keyup.enter="onSubmit(formRef)" :rules="rules" :model="state.form">
								...
								<el-form-item v-if="state.form.tab == 'register'">
									<el-radio-group size="large" v-model="state.form.registerType">
										<el-radio class="register-verification-radio" label="email" :disabled="!state.accountVerificationType.includes('email')" border>{{
											t('user.user.Via email') + t('user.user.register')
										}}</el-radio>
										<el-radio class="register-verification-radio" label="mobile" :disabled="!state.accountVerificationType.includes('mobile')" border>{{
											t('user.user.Via mobile number') + t('user.user.register')
										}}</el-radio>
									</el-radio-group>
								</el-form-item>
										...
										:placeholder="
											state.form.tab == 'register'
												? t('Please input field', {
														field: t('user.user.User name'),
												  })
												: t('Please input field', {
														field: t('user.user.account'),
												  })
										"
										:clearable="true"
										size="large"
									...
									<el-row class="w100">
										<el-col :span="16">
											<el-input
												...
												:placeholder="
													t('Please input field', {
														field: t('user.user.Verification Code'),
													})
												"
												size="large"
											>
										...
										<el-col class="captcha-box" :span="8">
											<img @click="onChangeCaptcha" class="captcha-img" :src="buildCaptchaUrl() + '&id=' + state.form.captchaId" alt="" />
										</el-col>
									</el-row>
								</el-form-item>
								...
								<el-form-item v-if="state.form.tab == 'register' && state.form.registerType == 'mobile'" prop="mobile">
									<el-input v-model="state.form.mobile" :placeholder="t('Please input field', { field: t('user.user.mobile') })" :clearable="true" size="large">
										<template #prefix>
								...
								<el-form-item v-if="state.form.tab == 'register' && state.form.registerType == 'email'" prop="email">
									<el-input v-model="state.form.email" :placeholder="t('Please input field', { field: t('user.user.mailbox') })" :clearable="true" size="large">
										<template #prefix>
									...
									<el-row class="w100">
										...
										<el-col class="captcha-box" :span="8">
											<el-button
												...
												type="primary"
												>{{ state.codeSendCountdown <= 0 ? t('user.user.send') : state.codeSendCountdown + t('user.user.seconds') }}</el-button
											>
										</el-col>
									</el-row>
								</el-form-item>
								...
								<div v-if="state.form.tab != 'register'" class="form-footer">
									...
									<div v-if="state.accountVerificationType.length > 0" @click="state.showRetrievePasswordDialog = true" class="forgot-password">
										{{ t('user.user.Forgot your password?') }}
									</div>
								</div>
								<el-form-item class="form-buttons">
									<el-button class="login-btn" @click="onSubmit(formRef)" :loading="state.formLoading" round type="primary" size="large">
										{{ t('user.user.' + state.form.tab) }}
									</el-button>
									<el-button v-if="state.form.tab == 'register'" @click="state.form.tab = 'login'" round plain type="info" size="large">{{
		...
		>
			<div class="retrieve-password-form">
				<el-form ref="retrieveFormRef" @keyup.enter="onSubmitRetrieve(retrieveFormRef)" :rules="retrieveRules" :model="state.retrievePasswordForm" :label-width="100">
					<el-form-item :label="t('user.user.Retrieval method')">
						<el-radio-group v-model="state.retrievePasswordForm.type">
							<el-radio label="email" :disabled="!state.accountVerificationType.includes('email')" border>{{ t('user.user.Via email') }}</el-radio>
							<el-radio label="mobile" :disabled="!state.accountVerificationType.includes('mobile')" border>{{ t('user.user.Via mobile number') }}</el-radio>
						</el-radio-group>
					</el-form-item>
					...
					<el-form-item prop="password" :label="t('user.user.New password')">
						<el-input v-model="state.retrievePasswordForm.password" :placeholder="t('Please input field', { field: t('user.user.New password') })" show-password>
							<template #prefix>
								<Icon name="fa fa-unlock-alt" size="16" color="var(--el-input-icon-color)" />
							</template>
						</el-input>
					...
					<el-form-item>
						...
						<el-button :loading="state.submitRetrieveLoading" @click="onSubmitRetrieve(retrieveFormRef)" type="primary">{{ t('user.user.second') }}</el-button>
					</el-form-item>
				</el-form>
			</div>
		</el-dialog>
